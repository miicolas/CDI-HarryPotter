...
export async function getFriends(req, res) {
  try {
    ...
    const usernamefriendsPending = await query(
      "SELECT DISTINCT username FROM Users JOIN Friends ON Users.id = Friends.userID2 WHERE Friends.userID1 = ? AND Friends.statut = 'pending'",
      [userId]
  );
